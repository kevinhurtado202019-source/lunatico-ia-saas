...
// manifest.json y sw.js (los de la propia app, y los de cada pagina
// publicada en /paginas/:codigo/) son los unicos .json/.js que si hacen
// falta servir de verdad -- todo lo demas con esas extensiones sigue
// bloqueado para no exponer server-saas.js, package.json, este mismo
// archivo, etc.
const PERMITIDOS_PWA = new Set(['/manifest.json', '/sw.js', '/.well-known/assetlinks.json']);
const PERMITIDO_PWA_PAGINA = /^\/paginas\/[^/]+\/(manifest\.json|sw\.js)$/;
const BLOQUEADOS = /\.(js|json|md|bat|ps1|lock|yml|yaml)$/i;
app.use((req, res, next) => {
    if (req.path.startsWith('/api/')) return next();
    if (PERMITIDOS_PWA.has(req.path) || PERMITIDO_PWA_PAGINA.test(req.path)) return next();
